...
const PTCycleSampleEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `PT Cycle Sample`, "", "", "PT Cycle Sample", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `pt-cycle-samples/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.ptcyclesample);
          }
          //set up data sources for relationship fields
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this PT Cycle Sample?",
      "Confirm submission",
    );
  ...
  const submitPTCycleSample = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `pt-cycle-samples/create`
        : `pt-cycle-samples/update/${pageConfig.Id}`;
...
export default PTCycleSampleEdit;