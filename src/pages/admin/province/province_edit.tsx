...
const ProvinceEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `Province`, "", "", "Province", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `provinces/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.province);
          }
          //set up data sources for relationship fields
          //set error
          setError(false);
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this Province?",
      "Confirm submission",
    );
  ...
  const submitProvince = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `provinces/create`
        : `provinces/update/${pageConfig.Id}`;
