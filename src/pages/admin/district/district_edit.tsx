...
const DistrictEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `District`, "", "", "District", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `districts/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.district);
          }
          //set up data sources for relationship fields
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this District?",
      "Confirm submission",
    );
  ...
  const submitDistrict = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `districts/create`
        : `districts/update/${pageConfig.Id}`;
