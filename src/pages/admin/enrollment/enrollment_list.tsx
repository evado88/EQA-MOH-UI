...
const AdminEnrollmentList = () => {
  ...
  const addButtonOptions = useMemo(
    () => ({
      icon: "add",
      text: "New Enrollment",
      onClick: () => navigate("/admin/enrollments/add"),
    }),
    [],
  );
        ...
        <Col sz={12} sm={12} lg={12}>
          <Card showHeader={false}>
            ...
            >
              ...
              <Column dataField="id" caption="ID" hidingPriority={11}></Column>
              <Column
                dataField="name"
                caption="Name"
                hidingPriority={10}
                ...
                cellRender={(e) => {
                  if (e.data.status.status_name == "Draft") {
                    return (
                    <Link to={`/admin/enrollments/edit/${e.data.id}`}>
                      {e.text}
                    </Link>
                  );
                  } else {
                    return (
                    <Link to={`/admin/enrollments/view/${e.data.id}`}>
                      {e.text}
                    </Link>
                  );
                  }
