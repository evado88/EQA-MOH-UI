...
const PTCycleStatusEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `PT Cycle Status`, "", "", "PT Cycle Status", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `pt-cycle-statuses/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.ptcyclestatus);
          }
          //set up data sources for relationship fields
          //set error
          setError(false);
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this PT Cycle Status?",
      "Confirm submission",
    );
  ...
  const submitPTCycleStatus = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `pt-cycle-statuses/create`
        : `pt-cycle-statuses/update/${pageConfig.Id}`;
...
export default PTCycleStatusEdit;