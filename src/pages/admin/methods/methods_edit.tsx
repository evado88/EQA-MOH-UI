...
const MethodsEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `Method`, "", "", "Method", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `methods/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.methods);
          }
          //set up data sources for relationship fields
          //set error
          setError(false);
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this Method?",
      "Confirm submission",
    );
