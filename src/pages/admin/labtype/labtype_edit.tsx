...
const LabTypeEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `Lab Type`, "", "", "Lab Type", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `lab-types/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.labtype);
          }
          //set up data sources for relationship fields
          //set error
          setError(false);
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this Lab Type?",
      "Confirm submission",
    );
  ...
  const submitLabType = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `lab-types/create`
        : `lab-types/update/${pageConfig.Id}`;
