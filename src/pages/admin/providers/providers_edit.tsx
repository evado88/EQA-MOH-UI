...
const ProvidersEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `Provider`, "", "", "Provider", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `providers/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.providers);
          }
          //set up data sources for relationship fields
          //set error
          setError(false);
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this Provider?",
      "Confirm submission",
    );
  ...
  const submitProviders = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `providers/create`
        : `providers/update/${pageConfig.Id}`;
