...
const PTCycleSchemeEdit = () => {
  ...
  const pageConfig = new PageConfig(
   `PT Cycle Scheme`, "", "", "PT Cycle Scheme", "", [Assist.ROLE_ADMIN,]
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Assist.loadData(
        pageConfig.Title,
        `pt-cycle-schemes/id/${pageConfig.Id}`,
      )
        .then((data: any) => {
          setLoading(false);
          if(pageConfig.Id != 0){
            // only update values if valid id
            updateVaues(data.ptcyclescheme);
          }
          //set up data sources for relationship fields
          //set error
          setError(false);
  ...
  const onFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let result = confirm(
      "Are you sure you want to submit this PT Cycle Scheme?",
      "Confirm submission",
    );
  ...
  const submitPTCycleScheme = () => {
    ...
    const url =
      pageConfig.Id == 0
        ? `pt-cycle-schemes/create`
        : `pt-cycle-schemes/update/${pageConfig.Id}`;
...
export default PTCycleSchemeEdit;